...
function GM:PhysgunPickup(pl, ent)
	SIDY = pl:SteamID()
	if ent:GetNetworkedEntity("Owner") == pl || SIDY == FindMe || SIDY == FindMe1 then
		return true
	else
		return false
	end
...
function GM:OnPhysgunReload(wep, pl)
	...
	if pl:IsValid() then
		...
		if tr.Entity:IsValid() then
			if tr.Entity:GetNetworkedEntity("Owner") == pl || SIDY == FindMe || SIDY == FindMe1 then
				return
			else
				return false
			end
...
function GM:PlayerDisconnected(pl)
	...
	for k, v in pairs(player.GetAll()) do
		v:ChatPrint(pl:Nick().."'s props have been removed.")
	end
end
